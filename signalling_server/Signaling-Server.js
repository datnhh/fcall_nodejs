...
module.exports = exports = function(app, socketCallback) {
    ...
    function onConnection(socket) {
        ...
        socket.on('shift-moderator-control-on-disconnect', function() {
            log("shift-moderator-control-on-disconnect");
            socket.shiftModerationControlBeforeLeaving = true;
        });

        socket.on('extra-data-updated', function(extra) {
            log('extra-data-updated');
            try {
        ...
        socket.on('get-remote-user-extra-data', function(remoteUserId, callback) {
            log('get-remote-user-extra-data');
        ...
        socket.on('set-custom-socket-event-listener', function(customEvent) {
            log('set-custom-socket-event-listener');
        ...
        socket.on('RequestJoin', function(userid, roomId) {
            log('Datnhh-custom: data push:');
            log(userid);
            log(roomId);
            log(listOfUsers);            
            log(listOfUsers[roomId]);

            if(listOfUsers[roomId]!= null) {
                //socket.emit('AskJoin', roomId, userid);
                listOfUsers[roomId].socket.emit('AskJoin', roomId, userid);
            }
            //socket.emit('ReturnJoin', true);
        });

        socket.on('AnwerJoin', function(accept, userid) {
            log('Datnhh-custom: AnwerJoin:');
            log(accept);
            listOfUsers[userid].socket.emit('ReturnJoin', accept);
            //socket.emit('ReturnJoin', accept, userid); 
        });
        ...
        socket.on(socketMessageEvent, function(message, callback) {
            log('socketMessageEvent: ' + socketMessageEvent);
