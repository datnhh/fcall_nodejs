function convertDatetoNumber(_date) {
    if (_date == null) _date = new Date();
    return _date.getTime();
}
function checkResponse(jsonData, divshowError) {
    try {        
        if (jsonData.StatusCode == 200) {
            return true;
        } else {
            if (jsonData.MessageResponse != undefined)
                alertify.error(jsonData.MessageResponse);
            else
                alertify.error(jsonData.Value);
        }        
    } catch (e) {
...
function checkResponseAngular(jsonData) {
    try {
        //return true;
        ...
        if (json.StatusCode == 200) {
            ...
            if (json.MessageResponse != undefined)
                alertify.error(json.MessageResponse);
            else
                alertify.error(json.Value);
        }
    } catch (e) {
        alertify.error(e.message);
    }
